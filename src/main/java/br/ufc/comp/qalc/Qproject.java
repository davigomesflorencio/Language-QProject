...
import br.ufc.comp.qalc.frontend.token.COMToken;
import br.ufc.comp.qalc.frontend.token.DIRTOKEN;
import br.ufc.comp.qalc.frontend.token.EOFToken;
import br.ufc.comp.qalc.frontend.token.WHITEToken;
 ...
 * <p>
 * Contém a função {@link Qproject#main(String[])} e as definições de interpretação
 * dos parâmetros de execução.
 */
@CommandLine.Command(
        versionProvider = VersionProvider.class,
        description = "Interpretador para a linguagem Qproject.",
        headerHeading = "Uso:%n",
        synopsisHeading = "\t",
            ...
            if (cmd.isUsageHelpRequested()) {
                ...
                switch (qalc.stopAt) {
                    case LEXER:
                        ...
                        NewTokenMessage MsgToken;

                        while (true) {
                            MsgToken = new NewTokenMessage(scannerInput.getNextToken());
                            if (MsgToken.getToken() instanceof EOFToken) {
                                break;
                            }
                            if (MsgToken.getToken() instanceof WHITEToken ||
                                    MsgToken.getToken() instanceof DIRTOKEN ||
                                    MsgToken.getToken() instanceof COMToken) {
                                continue;
                            } else {
